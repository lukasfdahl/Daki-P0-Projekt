...
def get_terrain(tile : np.ndarray):
    ...
    for y in range(5):
        for x in range(5):
            subtiles.append(tile[y*20:(y+1)*20, x*20:(x+1)*20])

    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        result = pool.map(run_subtile, subtiles)
        for list in result:
            for element in list:
                pixel_values.append(element)
...
def run_subtile(subtile : np.ndarray) -> list[str]:
    pixel_values = []

    rows, colums, _ = subtile.shape

    for y in range(rows):
        for x in range(colums):
            blue, green, red = subtile[x, y]
...
def calculate_closeness(check_color : tuple[int, int, int], target_color : tuple[int, int, int]) -> float:
    red_closeness = exp_function(abs(np.subtract(check_color[0], target_color[0])))
    blue_closeness = exp_function(abs(np.subtract(check_color[1], target_color[1])))
    green_closeness = exp_function(abs(np.subtract(check_color[2], target_color[2])))
